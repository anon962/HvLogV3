...
export function CountValueTable({
    label,
    rows,
    sectionClass,
    titleClass,
    layoutClass,
}: CountValueTableProps) {
    ...
    const sectionClasses = `count-value-table w-max ${
        sectionClass ?? ""
    }`
...
function SubTable({ rows }: { rows: CountValueTableSubRow[] }) {
    rows = sortBy(rows, [
        { fn: (r) => r.value },
        { fn: (r) => r.count },
        { fn: (r) => r.label, reverse: true },
...
export interface CountValueTableProps {
    label: string
    rows: CountValueTableRow[]
    sectionClass?: string
    titleClass?: string
    layoutClass?: string
}

export interface CountValueTableRowProps {
    onClick: () => void
    row: CountValueTableRow
    isActive: boolean
    isNextActive: boolean
    className?: string
}

export interface CountValueTableRow {
    label: string
    count: number
    value: number
    subRows?: CountValueTableSubRow[]
}

export interface CountValueTableSubRow {
    label: string
    count: number
    value: number
}
