import { isEqual, ISODate, L, newContext } from "myutils"
...
const USERSCRIPT_CONFIG_ = newContext(() => {
    ...
    useEffect(() => {
        async function run() {
            const update = await validateConfigUser(config)
    ...
    useEffect(() => {
        return DbN.listenIdbEvent((ev, { isSameTab }) => {
            ...
            L.log("Applying config change from other tab")
            setConfig(ev.config)
...
export const DEFAULT_USERSCRIPT_CONFIG = () => ({
    ...
    showDelete: "default" as "default" | "yes" | "warn" | "no",
    prefetchDelay: 0,

    hvdataUploadMode: "default" as
        | "default"
        | "disabled"
        | "manual"
        | "auto_new"
...
export async function validateConfigUser(
    ...
    if (x.hvdataNamePref) {
        x.hvdataNamePref = x.hvdataNamePref.replaceAll(/[^a-z0-9\s]/gi, "")
        x.hvdataNamePref = x.hvdataNamePref.trim()
        if (x.hvdataNamePref.length === 0) {
            x.hvdataNamePref = null
        }
    }

    if (x.hvdataUser) {
        x.hvdataUser.id = x.hvdataUser.id.trim()
        x.hvdataUser.key = x.hvdataUser.key.trim()
        if (x.hvdataUser.id.length === 0 || x.hvdataUser.key.length === 0) {
            x.hvdataUser = null
        }
    }

    if (x.hvdataUploadMode === "default" || x.hvdataUploadMode === "disabled") {
        return x
    }
